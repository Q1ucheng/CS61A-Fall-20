...
def pingpong(n):
    ...
    #             return decrease(n-1)
    #         else:
    #             return decrease(n-1) - 1
        
    # return increase(n)
    def calculate(index, step, result):
        if index == n:
            return result
        elif n == 1:
            return 1
            ...
            if num_eights(index) or (index % 8 == 0):
                return calculate(index + 1, -step, result - step)
            return calculate(index + 1, step, result + step)
...
def missing_digits(n):
    ...
    def calculate_num(n, result):
        if n == 0:
            return result
        if n > 0:
            if now(n) - forward(n) > 1:
                return calculate_num(n // 10, result + (now(n) - forward(n) - 1))
            else:
                return calculate_num(n // 10, result)
        
    
    return calculate_num(n, 0)  
