...
def roll_dice(num_rolls, dice=six_sided):
    ...
    points = [dice() for _ in range(num_rolls)]
    l = [x == 1 for x in points]
    return 1 if any(l) else sum(points)
    # END PROBLEM 1
...
def swine_align(player_score, opponent_score):
    ...
    if player_score == 0 or opponent_score == 0:
        return False
    return get_gcd(player_score, opponent_score) >= 10
    # END PROBLEM 4a
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say_high(score0, score1):
        def func(score_who):
            cur_running_high = score_who - last_score
            if cur_running_high > running_high and cur_running_high > 0:
                print(cur_running_high,'point(s)! The most yet for Player', who)
                return announce_highest(who, score_who, cur_running_high)
            else:
                return announce_highest(who, score_who, running_high)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    max_average = 0
    while num <= 10:
        average_tmp = make_averaged(roll_dice, trials_count)(num, dice)
        if average_tmp > max_average:
            max_average = average_tmp
            max_num = num
        num += 1
    return max_num
    # END PROBLEM 9
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    # if extra_turn(score, opponent_score) or (bacon_strategy(score, opponent_score, cutoff, num_rolls) and not extra_turn(score, opponent_score)):
    #     return 0
    # return num_rolls  # Replace this statement
    bacon_score = score + free_bacon(opponent_score)
    if extra_turn(bacon_score, opponent_score) == True:
        return 0
