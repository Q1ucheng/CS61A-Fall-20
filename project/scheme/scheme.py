...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        def link_to_list(link):
            res = []
            while link is not nil:
                res.append(link.first)
                link = link.rest
            return res
        new_frame = Frame(self)
        new_frame.bindings.update(zip(link_to_list(formals), link_to_list(vals)))
        return new_frame
        # END PROBLEM 10
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        func = do_lambda_form(lambda_expr, env)
        
        env.define(function_name, func)
...
def make_let_frame(bindings, env):
    ...
    bindings_list = link_to_list(bindings)

    for bindings_pair in bindings_list:
        validate_form(bindings_pair, 2, 2)
        name, value = bindings_pair.first, bindings_pair.rest
        value = eval_all(value, env)
